...
def main():
    ...
    pd_file_train = pd.read_csv(
        os.path.join(_PATH_RAW_DATA, "Corona_NLP_train.csv"), encoding="latin-1"
    )
    pd_file_test = pd.read_csv(
        os.path.join(_PATH_RAW_DATA, "Corona_NLP_test.csv"), encoding="latin-1"
    )
    ...
    pd_file_train["Sentiment"] = pd_file_train["Sentiment"].replace(
        {
            "Neutral": 2,
            "Positive": 3,
    ...
    pd_file_test = pd_file_test.drop(
        labels=["UserName", "ScreenName", "Location", "TweetAt"], axis=1
    )
    ...
    pd_file_test["Sentiment"] = pd_file_test["Sentiment"].replace(
        {
            "Neutral": 2,
            "Positive": 3,
            "Extremely Positive": 4,
    ...
    # PREPROCESS
    dataset_sentAnalysis_preprocessed = dataset_sentAnalysis.map(
        preprocessBatch, batched=True, batch_size=32
    )
    ...
    def tokenize(batch):
        return tokenizer(
            batch["text"], pad_to_max_length=True, truncation=True, max_length=max_len
        )

    dataset_sentAnalysis_encoded = dataset_sentAnalysis_preprocessed.map(
        tokenize, batched=True, batch_size=32
    )

    torch.save(
        dataset_sentAnalysis_encoded, os.path.join(_PATH_PROCESSED_DATA, "dataset.pth")
    )
